...
flowchart TD
    ...
    TaskQ -- Offline Trajectory Opt (Hard Constraints) --> DirCol["<b>Direct Collocation (Hermite-Simpson NLP)</b><br/>(Exact terminal equality satisfaction; fast SLSQP)<br/><i>[Exp 32]</i>"]
...
| **Offline Trajectory Optimization (Hard Endpoints)** | **Direct Collocation (Hermite-Simpson NLP)** | iLQR / Single Shooting / Sampling (CEM) | Single shooting (iLQR) and sampling (CEM) balance soft terminal penalty $Q_f$ against control effort, leaving non-zero terminal error ($0.253$ and $0.649$); Direct Collocation enforces exact equality constraints ($0.0$ planned, $8.1\times 10^{-4}$ re-integrated at 41 knots) and solves $2\times\text{--}12\times$ faster ($0.71\,\text{s}$) | [Exp 32](../experiments/32_direct_collocation_vs_ilqr/) |
...
### 3.13 Direct Collocation vs. Shooting vs. Sampling Trajectory Optimization
- **Hard Equality vs. Soft Penalty Formulation:** Direct transcription (Hermite-Simpson collocation) parametrizes state and control trajectories simultaneously as decision variables in a nonlinear program (NLP), enforcing system dynamics as collocation defect equalities and target boundaries as hard equality constraints ($x(0)=x_{\text{init}}, x(T)=x_{\text{goal}}$). In contrast, single shooting (iLQR) and sampling (CEM) must rely on quadratic terminal penalties $Q_f$. In minimum-effort swing-up ($T=2.0\,\text{s}$), collocation hits the target exactly ($0.0$ planned, $8.1\times 10^{-4}$ re-integrated), whereas iLQR and CEM stop $0.253$ and $0.649$ short because the optimizer balances soft terminal penalty against effort. Apparent lower effort for iLQR ($65.0$ vs $75.0$) is a relaxed problem artifact, not a win ([Exp 32](../experiments/32_direct_collocation_vs_ilqr/)).
- **Inter-Knot Quadrature Drift:** Collocation satisfies NLP equality constraints to $2.1\times 10^{-12}$, but Hermite-Simpson polynomial interpolation matches true physics only to third order across knot intervals. Re-integrating the open-loop plan through fine RK4 exhibits small numerical drift ($8.1\times 10^{-4}$ at 41 knots, $1.7\times 10^{-2}$ at 21 knots), which converges away with mesh refinement. Conversely, shooting and sampling roll true RK4 internally ($1\times 10^{-5}$ drift) but converge to the wrong endpoint under soft penalties ([Exp 32](../experiments/32_direct_collocation_vs_ilqr/)).
- **Compute Complexity & Offline vs. Online Roles:** Collocation solves the 247-variable NLP in $0.71\,\text{s}$ via SLSQP with analytic defect Jacobians, beating offline full-horizon iLQR ($1.41\,\text{s}$) and CEM ($8.55\,\text{s}$). For online receding-horizon control, 1-iteration RTI-iLQR remains the fastest choice ([Exp 24]), while Direct Collocation provides the ideal offline trajectory generator for feeding feedforward-feedback tracking architectures ([Exp 32]).

---

## 4. The 13 Invariant Engineering Laws Across 34 Experiments
...
12. **Underactuated Energy Pumping Unlocks Global Basins:** For underactuated pendulums (Cart-Pole, Furuta RIP), linear controllers cannot stabilize inverted hanging states ($\pm 180^\circ$); Lyapunov energy shaping drives mechanical energy monotonically to the homoclinic orbit, enabling reliable handoff to LQR / MPC ([Exp 07], [Exp 28]).
13. **Reporting Failure Modes is as Essential as Reporting Wins:** Engineering judgment relies on knowing exact breaking points: linearisation breakdown past $23^\circ$ ([Exp 02]), EKF false basin trapping ([Exp 16]), closed-loop SysID correlation bias ([Exp 09]), nominal computed torque collapse under payload step ([Exp 23]), pure RL bootstrap failure on low-inertia plants ([Exp 21]), Kinematic MPC overshoot under Pacejka tire saturation ([Exp 27]), Plain BC lane departure under high slip ([Exp 29]), cascaded PID droop on rolling ball friction ([Exp 33]), MRAC parameter drift divergence under unmatched force disturbances ([Exp 34]), and single-shooting terminal constraint relaxation under soft penalty balances ([Exp 32]).
