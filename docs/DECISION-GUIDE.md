...
- **When CEM wins (the mirror image):** On non-convex obstacle fields with moving disks, derivative-free CEM is the only entry that meaningfully avoids (36 collision steps vs 69 for iLQR and 82 for blind LQR) ([25](../experiments/25_diffdrive_moving_obstacle/)). Quartic obstacle barrier Hessians go non-convex near disk centres; 1-step RTI cannot clear the obstacle where derivative-free population sampling cuts a safe wide arc (at the price of $7\times$ energy).
- **When to use CEM:** Only when dynamics or costs are **non-smooth** (obstacle keep-outs, contact dynamics, non-differentiable rewards) where gradients do not exist or mislead ([20](../experiments/20_quadrotor_obstacle_nmpc/), [25](../experiments/25_diffdrive_moving_obstacle/)).
