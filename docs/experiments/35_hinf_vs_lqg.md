...
Companion: [`aimct.controllers.hinf`
(`mixsyn`, `weight_S` / `weight_T`, `StateSpace`),
[`aimct.controllers.solve_care` (the two LQG
Riccati equations), and the
[H-infinity reference](../references/hinf-reference.md).
...
![loop, S, T, and the margin bars](figures/exp35_figure.png)
